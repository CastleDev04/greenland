...
export default function PagosForm({
    ...
    if (pagoData) {
      ...
      if (pagoData.fechaPago) {
        try {
          ...
          if (!isNaN(fecha.getTime())) fechaPagoFormatted = fecha.toISOString().split('T')[0];
        } catch (error) {
          console.error('Error parsing fechaPago:', error);
        }
      }
      const isPendingCuota = !pagoData.tipoPago && !pagoData.fechaPago && pagoData.numero_cuota;
      return {
        ...
        fechaPago: fechaPagoFormatted || (isPendingCuota ? '' : new Date().toISOString().split('T')[0]),
  ...
  const tiposPago = ['efectivo', 'transferencia', 'cheque', 'tarjeta', 'deposito', 'giro'];
  ...
  const validateForm = () => {
    const newErrors = {};
    const isEditingPendingCuota = pagoData?.numero_cuota && !pagoData?.tipoPago && !pagoData?.fechaPago;
    const hasPartialPaymentInfo = !!formData.tipoPago || !!formData.fechaPago;

    if (!formData.venta_id) newErrors.venta_id = 'Debe seleccionar una venta';
    if (!formData.monto || formData.monto.trim() === '') newErrors.monto = 'El monto es requerido';
    else if (parseFloat(formData.monto) <= 0) newErrors.monto = 'El monto debe ser mayor a 0';
    else if (isNaN(parseFloat(formData.monto))) newErrors.monto = 'El monto debe ser un número válido';

    if (!formData.tipoPago && !formData.fechaPago) {
      if (!isEditingPendingCuota) {
        newErrors.tipoPago = 'Debe seleccionar un tipo de pago';
        newErrors.fechaPago = 'La fecha de pago es requerida';
      }
    } else if (hasPartialPaymentInfo && (!formData.tipoPago || !formData.fechaPago)) {
    ...
    if (formData.fechaPago) {
      ...
      if (fechaPago > hoy) newErrors.fechaPago = 'La fecha de pago no puede ser futura';
    }
  ...
  const calcularTotal = () => {
    return (parseFloat(formData.monto) || 0) + (parseFloat(formData.interes) || 0) + (parseFloat(formData.multa) || 0);
  };
  ...
  const getLoteInfo = (venta) => {
    ...
    if (venta.lote?.numero) return `Lote ${venta.lote.numero}`;
    if (venta.lote_numero) return `Lote ${venta.lote_numero}`;
    if (venta.lote_id) return `Lote #${venta.lote_id}`;
          ...
          <div className="p-6">
            <div className="flex justify-between items-start mb-6">
              <div>
                ...
                <p className="text-gray-600 mt-1">
                  {isEditing ? 'Modifique la información del pago' : 'Complete la información del pago recibido'}
                </p>
              </div>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <select name="venta_id" value={formData.venta_id} onChange={handleChange}
                      disabled={isLoading || isEditing}
                      className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors disabled:bg-gray-100 disabled:cursor-not-allowed ${errors.venta_id ? 'border-red-500 bg-red-50' : 'border-gray-300'}`}>
                    ...
                    {errors.venta_id && (
                      <p className="mt-1 text-sm text-red-600 flex items-center">
                        <AlertCircle size={14} className="mr-1" />{errors.venta_id}
                      </p>
                    )}
                  </div>
                    ...
                    <div className="bg-white p-3 rounded border border-blue-200">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                        ...
                        <div>
                          ...
                          <p className={`font-medium ${getSaldoPendiente(ventaSeleccionada) > 0 ? 'text-red-600' : 'text-green-600'}`}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input type="text" name="monto" value={formData.monto} onChange={handleChange} disabled={isLoading}
                        className={`w-full pl-12 pr-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors disabled:bg-gray-100 disabled:cursor-not-allowed ${errors.monto ? 'border-red-500 bg-red-50' : 'border-gray-300'}`}
                        placeholder="0.00" />
                    </div>
                    {errors.monto && <p className="mt-1 text-sm text-red-600 flex items-center"><AlertCircle size={14} className="mr-1" />{errors.monto}</p>}
                  </div>

                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">
                      <Calendar className="inline mr-1" size={14} />Fecha de Pago *
                    </label>
                    <input type="date" name="fechaPago" value={formData.fechaPago} onChange={handleChange}
                      disabled={isLoading} max={new Date().toISOString().split('T')[0]}
                      className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors disabled:bg-gray-100 disabled:cursor-not-allowed ${errors.fechaPago ? 'border-red-500 bg-red-50' : 'border-gray-300'}`} />
                    {errors.fechaPago && <p className="mt-1 text-sm text-red-600 flex items-center"><AlertCircle size={14} className="mr-1" />{errors.fechaPago}</p>}
                  </div>

                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">
                      <CreditCard className="inline mr-1" size={14} />Tipo de Pago *
                    </label>
                    <select name="tipoPago" value={formData.tipoPago} onChange={handleChange} disabled={isLoading}
                      className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors disabled:bg-gray-100 disabled:cursor-not-allowed ${errors.tipoPago ? 'border-red-500 bg-red-50' : 'border-gray-300'}`}>
                      <option value="">Seleccione tipo de pago</option>
                      {tiposPago.map((tipo) => (
                        <option key={tipo} value={tipo}>{tipo.charAt(0).toUpperCase() + tipo.slice(1)}</option>
                      ))}
                    </select>
                    {errors.tipoPago && <p className="mt-1 text-sm text-red-600 flex items-center"><AlertCircle size={14} className="mr-1" />{errors.tipoPago}</p>}
                  </div>

                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">
                      <FileText className="inline mr-1" size={14} />Comprobante (Opcional)
                    </label>
                    <input type="text" name="comprobante" value={formData.comprobante} onChange={handleChange}
                      ...
                      placeholder="Número o nombre del comprobante" maxLength={255} />
                  </div>
                </div>
              </div>
              ...
              <div className="bg-yellow-50 p-4 rounded-lg border border-yellow-200">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Interés Moratorio</label>
                    <div className="relative">
                      <span className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-500">Gs.</span>
                      <input type="text" name="interes" value={formData.interes} onChange={handleChange} disabled={isLoading}
                        className={`w-full pl-12 pr-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors disabled:bg-gray-100 disabled:cursor-not-allowed ${errors.interes ? 'border-red-500 bg-red-50' : 'border-gray-300'}`}
                        placeholder="0.00" />
                    </div>
                    {errors.interes && <p className="mt-1 text-sm text-red-600 flex items-center"><AlertCircle size={14} className="mr-1" />{errors.interes}</p>}
                  </div>

                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Multa por Mora</label>
                    <div className="relative">
                      <span className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-500">Gs.</span>
                      <input type="text" name="multa" value={formData.multa} onChange={handleChange} disabled={isLoading}
                        className={`w-full pl-12 pr-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors disabled:bg-gray-100 disabled:cursor-not-allowed ${errors.multa ? 'border-red-500 bg-red-50' : 'border-gray-300'}`}
                        placeholder="0.00" />
                    </div>
                    {errors.multa && <p className="mt-1 text-sm text-red-600 flex items-center"><AlertCircle size={14} className="mr-1" />{errors.multa}</p>}
                  </div>
                </div>
              </div>
